...
class SdkResolver {
  ...
  @protected
  void verbose(String msg) => _onVerbose?.call(msg);
  ...
  String resolveDartSdk({String? explicitPath}) {
    if (explicitPath != null) {
      verbose('[sdk] Dart SDK: explicit path → $explicitPath');
    ...
    if (discovered == null) {
      verbose('[sdk] Dart SDK: not found');
  ...
  String resolveFlutterSdk({required String explicitPath}) {
    verbose('[sdk] Flutter SDK: explicit path → $explicitPath');
    ...
    verbose('[sdk] Flutter SDK: embedded Dart $dartVersion validated');
    return explicitPath;
  }
  ...
  String _validateDartSdk(String sdkPath) {
    ...
    verbose('[sdk] Dart SDK: validated $version (satisfies $_requiredDartSdk)');
    return sdkPath;
  }
  ...
  String? _discoverDartSdk() {
    ...
    verbose('[sdk] Dart SDK: trying FVM config...');
    final fvmDartSdk = _dartSdkFromFvm();
    if (fvmDartSdk != null) {
      verbose('[sdk] Dart SDK: FVM config → $fvmDartSdk');
      return fvmDartSdk;
    }

    // 2. Try DART_SDK environment variable.
    verbose('[sdk] Dart SDK: trying DART_SDK env...');
    final dartSdk = Platform.environment['DART_SDK'];
    if (dartSdk != null && Directory(dartSdk).existsSync()) {
      verbose('[sdk] Dart SDK: DART_SDK env → $dartSdk');
      return dartSdk;
    }
    verbose('[sdk] Dart SDK: DART_SDK not set');

    // 3. Try which/where dart on PATH.
    verbose('[sdk] Dart SDK: trying which dart...');
    final sdkPath = _dartSdkFromWhich('dart');
    if (sdkPath != null) {
      verbose('[sdk] Dart SDK: which dart → $sdkPath');
      return sdkPath;
    }
    verbose('[sdk] Dart SDK: which dart failed');
  ...
  String? _dartSdkFromFvm() {
    ...
    while (true) {
      ...
      if (fvmrc.existsSync()) {
        ...
        if (match != null) {
          verbose('[sdk] Dart SDK: .fvmrc → Flutter ${match.group(1)}');
          final dartSdk = _fvmCacheDartSdk(match.group(1)!);
          if (dartSdk != null) return dartSdk;
          verbose('[sdk] Dart SDK: FVM cache miss for ${match.group(1)}');
        }
      }
      ...
      if (config.existsSync()) {
        ...
        if (match != null) {
          verbose('[sdk] Dart SDK: .fvm/fvm_config.json → Flutter ${match.group(1)}');
          final dartSdk = _fvmCacheDartSdk(match.group(1)!);
          if (dartSdk != null) return dartSdk;
          verbose('[sdk] Dart SDK: FVM cache miss for ${match.group(1)}');
        }
