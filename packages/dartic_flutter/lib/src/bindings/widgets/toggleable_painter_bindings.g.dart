...
class _$ToggleablePainter extends ToggleablePainter implements DarticObjectHolder {
  ...
  void paintRadialReaction({required Canvas canvas, Offset offset = Offset.zero, required Offset origin}) {
    final _$r = _dispatch.invoke($darticObject.bridge ?? $darticObject, $darticObject, 'paintRadialReaction', [canvas, offset, origin]);
    if (identical(_$r, notOverridden)) { super.paintRadialReaction(canvas: canvas, offset: offset, origin: origin); return; }
  }
