...
class _$FixedExtentScrollController extends FixedExtentScrollController implements DarticObjectHolder {
  ...
  Future<void> animateToItem(int itemIndex, {required Duration duration, required Curve curve}) {
    final r = _dispatch.invoke($darticObject.bridge ?? $darticObject, $darticObject, 'animateToItem', [itemIndex, duration, curve]);
    if (identical(r, notOverridden)) return super.animateToItem(itemIndex, duration: duration, curve: curve);
    return r as Future<void>;
  }

  @override
  void jumpToItem(int itemIndex) {
    final r = _dispatch.invoke($darticObject.bridge ?? $darticObject, $darticObject, 'jumpToItem', [itemIndex]);
    if (identical(r, notOverridden)) { super.jumpToItem(itemIndex); return; }
  }
