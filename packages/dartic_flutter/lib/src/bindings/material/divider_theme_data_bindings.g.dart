...
class _$DividerThemeData extends DividerThemeData implements DarticObjectHolder {
  ...
  DividerThemeData copyWith({Color? color, double? space, double? thickness, double? indent, double? endIndent, BorderRadiusGeometry? radius}) {
    final r = _dispatch.invoke($darticObject.bridge ?? $darticObject, $darticObject, 'copyWith', [color, space, thickness, indent, endIndent, radius]);
    if (identical(r, notOverridden)) return super.copyWith(color: color, space: space, thickness: thickness, indent: indent, endIndent: endIndent, radius: radius);
