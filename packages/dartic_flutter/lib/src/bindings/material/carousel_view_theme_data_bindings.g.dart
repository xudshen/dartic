...
class _$CarouselViewThemeData extends CarouselViewThemeData implements DarticObjectHolder {
  ...
  CarouselViewThemeData copyWith({Color? backgroundColor, double? elevation, OutlinedBorder? shape, WidgetStateProperty<Color?>? overlayColor, EdgeInsets? padding, Clip? itemClipBehavior}) {
    final r = _dispatch.invoke($darticObject.bridge ?? $darticObject, $darticObject, 'copyWith', [backgroundColor, elevation, shape, overlayColor, padding, itemClipBehavior]);
    if (identical(r, notOverridden)) return super.copyWith(backgroundColor: backgroundColor, elevation: elevation, shape: shape, overlayColor: overlayColor, padding: padding, itemClipBehavior: itemClipBehavior);
