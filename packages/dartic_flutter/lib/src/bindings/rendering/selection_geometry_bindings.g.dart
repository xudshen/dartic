...
class _$SelectionGeometry extends SelectionGeometry implements DarticObjectHolder {
  ...
  SelectionGeometry copyWith({SelectionPoint? startSelectionPoint, SelectionPoint? endSelectionPoint, List<Rect>? selectionRects, SelectionStatus? status, bool? hasContent}) {
    final r = _dispatch.invoke($darticObject.bridge ?? $darticObject, $darticObject, 'copyWith', [startSelectionPoint, endSelectionPoint, selectionRects, status, hasContent]);
    if (identical(r, notOverridden)) return super.copyWith(startSelectionPoint: startSelectionPoint, endSelectionPoint: endSelectionPoint, selectionRects: selectionRects, status: status, hasContent: hasContent);
