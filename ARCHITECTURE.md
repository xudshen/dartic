...
AOT 原生代码的 **1/5 ~ 1/3**。关键手段：寄存器式字节码、`Uint32List` 紧凑指令、IC + Quickening、共享 Buffer 双视图值栈。

## 编译与执行流程

```
Dart 源码 ──CFE──► .dill (Kernel AST) ──darti 编译器──► .dartib (字节码)
                                              │
                                        Bridge 代码（自动生成）
                                              │
                                              ▼
...
| Ch4 | `docs/design/04-compiler.md` | Kernel 遍历、LSRA、闭包编译、.dartib 格式 |
