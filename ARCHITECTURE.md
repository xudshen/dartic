...
`dartic` CLI 提供统一的命令行入口：

```
dartic compile lib/app.dart -o app.darb   # 编译 .dart → .darb
dartic run app.darb                        # 执行 .darb 字节码
dartic run lib/app.dart                    # 一步编译+执行
dartic gen dartic_export.yaml              # 生成 Bridge 绑定代码
dartic doctor                              # 环境诊断
dartic version                             # 版本信息
```

架构分为两层：
- **核心层** `lib/src/toolchain/` — SDK 发现、Target 检测、编译管线，不依赖 CLI 框架
- **CLI 壳子** `packages/dartic_cli/` — 参数解析、命令路由、输出格式化

## 架构分层

```
┌─────────────────────────────────────────────────┐
│  dartic CLI      compile · run · gen · doctor    │  packages/dartic_cli/
├─────────────────────────────────────────────────┤
│  Toolchain 核心   SDK 发现 · Target 检测 · 编译管线 │  lib/src/toolchain/
├─────────────────────────────────────────────────┤
│  宿主 Dart VM                                    │
├─────────────────────────────────────────────────┤
...
- **package:args** + **package:mason_logger** — CLI 框架 (dartic_cli)
- **package:yaml** — pubspec 解析、Target 自动检测
