...
| 输入 | Ch1 ISA | 分发循环解码并执行 Ch1 定义的 64 位定宽指令 |
...
| 属性 | 类型 | 说明 |
|------|------|------|
| bytecode | Uint64List | 64 位定宽指令序列（Ch1） |
...
| invokeClosure | (DarticClosure, List\<Object?\> args) -> Object? | VM 回调解释器闭包的入口（详见"invokeClosure 机制"节） | Ch4 ClosureAdapter、BridgeMixin |
...
**fuel 共享不变式**：当 VM 通过 ClosureAdapter 回调解释器时（详见 Ch4），`invokeClosure` 内部调用 `_driveInterpreter()` 与外层分发循环**共享同一回合的 fuel 预算**，不分配独立预算。这保证了回调中的无限循环同样受 fuel 保护，不会饿死宿主事件循环。
...
| 指令 | 目标确定方式 | 备注 |
|------|-------------|------|
| CALL A, B, C | `refStack[B]` 为 DarticClosure → 提取 funcProto + upvalues | 闭包调用，上值表存入新帧；若 `closure.boundFTA != null`（由 `BIND_CLOSURE_FTA` 设置），自动写入 `rBase+1`（FTA 槽位） |
...
`invokeClosure` 是 VM 回调解释器的统一入口，供 Ch4 的 ClosureAdapter 和 BridgeMixin 使用。当 VM 侧代码（如 `list.map(callback)`）触发解释器闭包时，调用链为：VM API -> ClosureAdapter.proxyN() -> `_runtime.invokeClosure(closure, args)`。
...
1. **字节码验证**（委托 Ch8 沙箱层）：操作码合法性、操作数范围、跳转目标有效性
