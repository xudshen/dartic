...
**工厂/构造函数的 null-padding 调用约定**：对于 factory 构造函数和平台类构造函数，编译器将所有可选命名参数和可选位置参数用 null 填充，使 `CALL_HOST` 的 arity 始终等于参数最大数量。生成的 methodMap entry 通过 `args.length > N && args[N] != null` 检查来区分哪些可选参数被提供。这避免了命名参数歧义问题——不同的命名参数组合在相同 arity 下无法仅靠参数数量区分（例如 `Duration(hours: 1)` 和 `Duration(minutes: 1)` 的位置参数数量相同，但需要路由到不同的构造逻辑）。
...
**注册顺序无关**：`HostDispatchRegistry.register()` 接受必填 `type` 参数和可选 `test` 参数，注册时直接写入 `_exactMap`（精确类型 → dispatcher）。多个独立 Plugin 注册宿主类时无需保证调用顺序——`_exactMap` 预热使精确类型 O(1) 直达，predicate scan 仅作为未注册子类型的 fallback。此设计与业界 VM 对齐（Dart VM CID / JVM klass / CPython ob_type 均使用精确类型标识做分发）。

**Object 分发注意事项**：代码生成器为 Object 注册 dispatcher 时**不传 `test:` 参数**（仅注册 `type: Object`）。这是因为 `(o) => o is Object` 对任意对象都返回 true，而 `HostDispatchRegistry.lookup()` 使用反向扫描（从最后注册的 entry 向前扫描），如果 Object 的 predicate 存在，它会在扫描时遮蔽所有更具体的类型（如 int、String、List 等），导致所有动态分发都路由到 Object 的 dispatcher。去掉 `test:` 后，Object 只通过 `_exactMap[Object]` 精确匹配（仅当 `receiver.runtimeType == Object` 时命中），不会干扰子类型的 predicate scan。
...
### BridgeDispatch（Bridge 方法分发）

BridgeDispatch 实现 `DarticRuntime` 接口，为 Bridge 实例提供将虚方法/属性调用路由回解释器的能力。当 VM 侧调用 Bridge 重写的方法时，`$BridgeMixin` 通过持有的 `DarticRuntime`（即 BridgeDispatch 实例）将调用转发给解释器的分发循环。

**实现机制**：BridgeDispatch 持有 `DarticModule` 引用和 `CallDarticMethod` 回调（绑定到 `DarticInterpreter._callDarticMethod`）。使用 `CallDarticMethod` 回调而非直接引用 DarticInterpreter，避免了跨库访问私有方法（`_runNestedDispatch`）的问题。

| 方法 | 行为 |
|------|------|
| `invoke(self, method, args)` | 通过 `ConstantPool.lookupNameIndex` 解析方法名为 nameIdx → 查 `DarticClassInfo.methods[nameIdx]` → 命中则通过 `CallDarticMethod` 回调执行解释器方法 → 未命中返回 `bridgeNotOverridden` 哨兵 |
...
**`bridgeNotOverridden` 哨兵**：`const Symbol #_bridgeNotOverridden`，是包内私有 Symbol，脚本代码无法构造。Bridge 生成代码通过 `identical(result, bridgeNotOverridden)` 判断脚本是否重写了该方法——若未重写则回退到 `super.method()` 调用。

**NEW_INSTANCE 集成**：`NEW_INSTANCE` 指令创建 `DarticObject` 后，以 `classId` 在 `BridgeFactoryRegistry` 中查找工厂。命中时，将当前活跃的 `BridgeDispatch` 实例和 `DarticObject` 传递给工厂，工厂返回 Bridge 实例。引用栈中存储 Bridge 实例（替代 DarticObject）。
...
#### ScriptObjectHolder 接口

`ScriptObjectHolder` 是所有 Bridge 类实现的接口，暴露 `$darticObject` getter，使解释器能从 Bridge 实例中提取内嵌的 DarticObject。
...
该接口是 Bridge 与解释器之间的契约——解释器不需要知道具体的 Bridge 类型，只需通过 `is ScriptObjectHolder` 检查即可提取脚本对象。

#### DarticRuntime API（双参数设计）

BridgeDispatch 的 `invoke` / `get` / `set` 方法接收两个参数：`receiver`（原始接收者，Bridge 实例）和 `scriptObject`（提取后的 DarticObject）。双参数设计的原因：

- `scriptObject`（DarticObject）用于在 `DarticClassInfo.methods` 中查找方法、在 ConstantPool 中解析名称
...
2. 若对象实现 ScriptObjectHolder → 返回 `$darticObject`
3. 否则抛出 DarticError
...
| ScriptObjectHolder（Bridge） | 提取 $darticObject → 从 DarticClassInfo.methods 查找 → 将 Bridge 实例作为 this 传入方法帧 |
| 宿主对象 | 走 HostDispatchRegistry 动态分发 |
...
当 Bridge 实例执行 CALL_SUPER 时，需要从 Bridge 接收者中提取 DarticObject 以查找 ITA（Inherited Type Arguments，继承类型参数）。解释器在 CALL_SUPER 路径中检查当前帧的 `this`（寄存器 0），若为 ScriptObjectHolder 则提取 $darticObject 用于 ITA 查找。
