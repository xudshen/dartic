...
> **Phase 2**：ClosureAdapter 身份缓存。当前每次 `proxyN()` 创建新 Dart 闭包，`removeListener` 等需要同一 Function 实例的 API 无法正常工作。可在 DarticProxyManager 中为 DarticClosure → Dart Function 维护 Expando 缓存，保证同一 DarticClosure 的同参数数量包装产生同一 Dart Function 实例。触发条件：Flutter `addListener`/`removeListener` 场景因闭包身份不一致导致监听器泄漏。
