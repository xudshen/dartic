...
| dispatch | DarticDispatch? | 方法分发引用，`NEW_INSTANCE` 时设置，使 `toString`/`==`/`hashCode` 能调用 dartic 覆盖 |
| bridge | Object? | 主 Bridge 实例（由 `WRAP_BRIDGE` 设置），所有 bridge dispatch 使用此字段作为 receiver |
| _faces | Map\<int, Object\>? | 接口 Bridge 缓存：classId → face 实例，由 `EXTRACT_FACE` 懒创建（详见 Ch4） |
