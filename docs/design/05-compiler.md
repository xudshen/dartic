...
调用端编译：`ConstructorInvocation` 编译为 `NEW_INSTANCE A, Bx`（分配对象）→ 编译参数 → `CALL_STATIC`（调用构造函数，传入新对象作为 receiver）。若类有宿主超类（`hostSuperClassName != null`），在 `CALL_STATIC` 之后追加 `WRAP_BRIDGE A, Bx`（查 BridgeFactoryRegistry，命中则用 Bridge 替换 DarticObject）。
