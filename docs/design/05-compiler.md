...
| 输出目标 | Ch1 ISA | 生成符合 Ch1 编码格式的 `Uint64List` 字节码 |
...
|----------|-----------|-----------------------------------------------|
| `InterfaceType` | int/double/bool → value，其他 → ref | 编译为字节码的类 → InterfaceTypeTemplate(classId, [typeArg...])；宿主库类 → HostClassTypeTemplate(fqn, [typeArg...])，运行时通过 HostTypeResolver 解析 classId。Nullability 影响 `is`/`as` 检查 |
...
| `RecordType` | ref | 已实现 RecordTypeTemplate(positionalTypes, namedTypes)，运行时 resolveType 为 DarticRecordType |
...
编译器需要区分"编译为字节码的用户代码"和"生成 CALL_HOST 的宿主代码"。CFE 输出的 `.dill` 包含所有依赖的完整 AST（用户代码 + 宿主 app + Flutter + dart:*），编译器必须正确分类。

编译器接受 `compilablePackages: Set<String>` 参数，采用**反转模型**：声明可编译的包，其余默认为 host：

```dart
bool _isHostLibrary(ir.Library lib) {
  final uri = lib.importUri;
  if (uri.isScheme('dart')) return true;            // dart:* 永远是宿主
  if (uri.isScheme('file')) return false;            // 用户入口文件，编译
...
| Kernel 节点 | 编译策略 |
|------------|----------|
| `IntLiteral` | 小整数（-2147483647 ~ +2147483648）：`LOAD_INT A, sBx`（立即数，零常量池访问）；大整数：`LOAD_CONST_INT A, Bx`（值栈 intView） |
...
| `Instantiation` | 编译函数操作数 → 将类型参数逐个 `INSTANTIATE_TYPE` → `CREATE_TYPE_ARGS` 打包为 `List<DarticType>` → `BIND_CLOSURE_FTA` 绑定到闭包对象的 `boundFTA` 字段；调用时运行时将 boundFTA 传入被调函数的 FTA 槽位 |
...
5. **Super 初始化器**（`SuperInitializer`）：若 super 构造函数在 dartic 类中，编译参数 → `CALL_SUPER`；若 super 构造函数在宿主类中，编译所有参数（位置 + 命名，按目标声明顺序，缺省参数填充默认值）→ `STORE_SUPER_ARGS`（存入 `DarticObject.pendingSuperArgs`）。每个构造函数最多一个
6. **重定向初始化器**（`RedirectingInitializer`）：编译参数 → `CALL_STATIC` 到目标构造函数，排斥其他初始化器
7. **构造函数体**（`function.body`）：正常编译函数体语句

调用端编译：`ConstructorInvocation` 编译为 `NEW_INSTANCE A, Bx`（分配对象）→ 编译参数 → `CALL_STATIC`（调用构造函数，传入新对象作为 receiver）。若类有宿主超类（`hostSuperClassName != null`）或宿主接口（`hostInterfaceNames != null`），在 `CALL_STATIC` 之后追加 `WRAP_BRIDGE A, Bx`（查 BridgeFactoryRegistry，命中则用 Bridge 替换 DarticObject）。注：`hostSuperClassName` 的检测会穿透 mixin application 中间类（沿 Kernel IR `superclass` 链向上找到第一个 host class），因此 `class A extends Error with Mx` 也能正确触发 WRAP_BRIDGE。

### 闭包编译

编译器遍历变量引用，标记被内层函数捕获的变量为 `isCaptured`。上值描述符记录来源：是直接捕获外层局部变量（`isLocal = true`，记录外层寄存器索引），还是间接通过外层上值链传递（`isLocal = false`，记录外层上值索引）。

**`this` 捕获**：`ThisExpression` 不是变量声明，需要独立的捕获分析。`_CapturedVarVisitor` 在遍历闭包体时检测 `ThisExpression` 并设置 `capturesThis` 标志。当闭包捕获 `this` 时，编译器添加一个指向外层方法 `rsp+2`（`this` 槽位）的上值描述符（`isLocal = true, index = 2`）。嵌套闭包通过 `isLocal = false` 传递链实现 `this` 的间接捕获。`_compileThisExpression` 在检测到 `_thisUpvalueIdx >= 0` 时发射 `LOAD_UPVALUE` 替代直接读取 `rsp+2`。

`CLOSURE A, Bx` 指令在运行时根据 `funcProto[Bx]` 的上值描述符列表，从当前帧的寄存器或上值表中收集上值，创建闭包对象。

当变量离开作用域且被捕获时，编译器在作用域退出前插入 `CLOSE_UPVALUE A`，运行时将所有指向 >= A 槽位的开放上值关闭（值从栈复制到上值对象内部）。编译器在以下作用域退出点发射 `CLOSE_UPVALUE`：函数返回、`Block` 退出、`for` 循环退出。`_emitCloseUpvaluesForScope()` 扫描当前 scope 的 `localDeclarations`，找到被捕获变量中最小的引用栈寄存器编号作为 `CLOSE_UPVALUE` 的操作数。当 `this` 被内层闭包捕获时（`_thisCapturedByInner = true`），`_emitCloseUpvaluesIfNeeded` 同样发射 `CLOSE_UPVALUE`，确保方法返回前关闭 `this` 的上值。
...
**泛型实例化**（`Instantiation`）：当泛型函数被实例化（如 `foo<int>`）时，编译器先 `CLOSURE` 创建闭包，然后将类型参数编译为 `List<DarticType>`，最后 `BIND_CLOSURE_FTA A, B` 将类型参数列表绑定到闭包的 `boundFTA` 字段。运行时调用该闭包时，自动将 `boundFTA` 传入被调函数的 FTA 槽位（`rBase+1`）。
...
| 约束 | 限制值 | 来源 |
|------|--------|------|
| 寄存器编号 | 0-65535（16 位） | Ch1 ABC 编码格式中 A/B/C 各 16 位 |
| 本地绑定索引 | 0-4294967295（32 位） | Ch1 ABx 编码格式中 Bx 32 位 |
| 跳转偏移（有符号） | -2147483647 ~ +2147483648 | Ch1 AsBx 编码格式，excess-K（32 位） |
| 常量池索引 | 0-4294967295（32 位 Bx）或 0-281474976710655（48 位 Ax） | ABx 格式 32 位，Ax 格式 48 位 |
| 函数参数数量 | ≤ 65535 | 16 位寄存器编码限制 |
| 异常处理器表大小 | 每函数无硬上限，但嵌套深度受寄存器数约束 | 编译器按 (startPC, endPC) 排序保证内层优先匹配 |
| 闭包上值数量 | 每闭包 ≤ 65536 个上值 | 上值索引 16 位编码 |
| 全局变量数量 | 0-4294967295 | 32 位索引 |
