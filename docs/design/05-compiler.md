...
5. **Super 初始化器**（`SuperInitializer`）：若 super 构造函数在脚本类中，编译参数 → `CALL_SUPER`；若 super 构造函数在宿主类中，编译所有参数（位置 + 命名，按目标声明顺序，缺省参数填充默认值）→ `STORE_SUPER_ARGS`（存入 `DarticObject.pendingSuperArgs`）。每个构造函数最多一个
