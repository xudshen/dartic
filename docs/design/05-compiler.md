...
编译器接受 `hostPackages: Set<String>` 参数，结合硬编码的 `dart:*` 判定：
...
`hostPackages` 通过 `discoverHostPackages()` 工具函数从 plugin 包目录的 `dartic.manifest` 文件自动发现，也支持手动指定。详见 `docs/plans/2026-03-07-host-library-manifest-design.md`。
