...
| 输入来源 | Ch1 ISA | 验证器依据 ISA 定义的操作码合法性、编码格式进行检查 |
...
| 回调重入中的无限循环 | CALL_HOST -> ClosureAdapter -> 解释器循环 | 回调共享当前回合 fuel，不分配独立预算（Ch3 fuel 共享不变式） | 运行时 |
...
- 回调重入（`CALL_HOST` -> ClosureAdapter -> 解释器）**共享当前回合的 fuel**，不分配独立预算（详见 Ch3 "fuel 共享不变式"）
- 回调中的无限循环同样受 fuel 保护
