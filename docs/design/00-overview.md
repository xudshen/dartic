...
**插件系统**：为 Dart/Flutter 应用提供安全、可控的第三方代码执行能力，支持插件化扩展和沙箱化运行。
...
2. **ISA 定宽编码**：所有指令固定 64 位，`Uint64List` 存储。操作数字段足够宽，无需 WIDE 前缀（Ch1）
...
| 指令存储 | `List<EvcOp>` 对象列表 | AST 树 | 字节码 | **`Uint64List` 紧凑编码** |
