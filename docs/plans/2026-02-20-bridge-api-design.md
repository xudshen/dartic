...
| 依赖 | Ch4 跨边界互调 | 使用 HostBindingRegistry（绑定注册）、HostClassRegistry（动态分发）、DarticProxyManager（代理管理）、BridgeFactory 签名 |
...
对比了 5 个嵌入式引擎的 API 设计：

| 引擎 | 语言 | 通信模型 | 类型安全 | dartic 可继承宿主类 | 代码生成 |
|------|------|---------|---------|-----------------|---------|
...
- Bridge 模式允许 dartic 继承宿主类（与 dart_eval 类似）
...
| 决策项 | 选择 | 备选方案与拒绝理由 | 理由 |
|--------|------|-------------------|------|
| API 风格 | Config + Engine（Flutter 风格） | Flat Engine：缺乏渐进式复杂度；Runtime + Dartic 分离：概念过多 | Dart/Flutter 开发者熟悉 config 模式 |
...
| onError 作用范围 | 仅处理 dartic 未捕获异常 | 统一处理所有错误：资源错误（fuel/timeout）应始终传播到宿主 | 资源违规是不可恢复错误，宿主必须感知 |
| engine.call() 重入 | 宿主回调内可再次调用 | 禁止重入：限制 host→dartic→host 链场景 | 嵌套 drive 机制已支持重入（详见 Ch3） |
| HostClassRegistry 生命周期 | DarticEngine 持有，传入解释器 | 解释器 per-execute 内部创建：registerClass 的注册会丢失 | 持久注册表与引擎生命周期一致 |
...
| call | (String function, [List\<Object?\> args]) → Object? | 按名称调用 dartic 顶层函数。async 函数返回 Future。支持重入 |
...
| onPrint | void Function(Object?)? | null（静默丢弃） | dartic print() 的处理器 → 映射到 CoreBindings.registerAll(printFn:) |
| onUnhandledException | void Function(Object, StackTrace)? | null（直接抛出） | **仅处理 dartic 未捕获异常**（详见错误模型节） |
...
Bridge 模式允许 dartic 类继承宿主类——解决"实例需同时是真正的 VM 类型（`is` 检查通过）和解释器对象（有 dartic 字段，虚方法可 override）"的矛盾。内部实现详见 Ch4 Bridge 类与 $BridgeMixin。
...
| 参数 2 | DarticObject：dartic 侧对象，承载 dartic 定义的字段 |
...
| 方法 | 签名 | 说明 |
|------|------|------|
| invoke | (DarticObject self, String method, List\<Object?\> args) → Object? | 分发虚方法/操作符调用。dartic 未 override → 返回 notOverridden |
| get | (DarticObject self, String property) → Object? | 分发属性 getter。dartic 未 override → 返回 notOverridden |
| set | (DarticObject self, String property, Object? value) → void | 分发属性 setter |

**notOverridden 哨兵值**：`#_notOverridden`（dartic 包内的私有 Symbol）。因为是私有符号，dartic 代码无法构造此值，`identical()` 比较无误报。Bridge 委托方法检测到 notOverridden 后调用 `super.xxx()` 回退到宿主实现。
...
3. Super 转发器方法（`$super$methodName()`），注册到 HostBindingRegistry，编译器对 VM super 目标生成 `CALL_HOST`

### 数据交换规则

| 宿主侧 | dartic 侧 | 方向 | 包装方式 |
|---------|--------|------|----------|
...
| 宿主枚举 | dartic 使用 | 宿主→dartic | 无（VM 对象直接存引用栈） |
| dartic 枚举 | 宿主接收 | dartic→宿主 | DarticProxy（与其他 dartic 定义对象一致） |
| @DarticExport 标注类 | dartic 使用 | 宿主→dartic | HostBindingRegistry 绑定 + HostClassRegistry 动态分发 |
| dartic 定义类 | 宿主接收 | dartic→宿主 | DarticProxy（自动，Expando 缓存保证身份一致性） |
| 宿主 Function | dartic 调用 | 宿主→dartic | HostBindingRegistry 注册 |
| dartic 闭包 | 宿主调用 | dartic→宿主 | ClosureAdapter（proxy0 ~ proxy6，详见 Ch4） |
| @DarticExport(bridge: true) | dartic extends | 双向 | Bridge 类（codegen 生成，VM `is` 检查通过） |
| Future\<T\>（async 函数） | engine.call() 返回 | dartic→宿主 | 返回 VM Future（Completer 桥接，详见 Ch7） |

**可空类型**：null 按上表 null 行处理，非 null 值按基础类型处理。无需额外包装。

**dartic 异常跨边界**：dartic throw 的自定义类型到宿主侧为 DarticProxy，`on SomeType catch` 无法匹配。需要按类型捕获时，dartic 应 throw 宿主 VM 已知的异常类型（详见 Ch4 异常跨边界传播契约）。
...
| 场景 | 行为 |
|------|------|
| dartic throw，dartic 未捕获 | 若 onUnhandledException 非 null → 调用 onUnhandledException，engine.call() 返回 null；否则原始异常传播到宿主 |
| 宿主函数 throw | dartic catch 可捕获（VM 异常类型保留，`on FormatException` 可匹配） |
...
| call() | DarticError 子类（资源限制）、dartic 异常（经 onUnhandledException 或直接传播） |
...
1. **绑定注册**：遍历 methods map，对每个条目调用 `HostBindingRegistry.register('$name::$key', wrapper)`
2. **动态分发注册**：调用 `HostClassRegistry.register(test, [name], exactType: type)`，使 `GET_FIELD_DYN` / `SET_FIELD_DYN` 指令能按类型路由到正确的分发器。若 `type` 非 null，dispatcher 同时写入 `_exactMap` 实现 O(1) 精确类型分发
3. **Bridge 工厂注册**（可选）：若 bridgeFactory 非 null，存入 `BridgeFactoryRegistry(name → factory)`

**Phase 7.1 变更**：HostClassRegistry 重构为两层查找策略：① `_exactMap[runtimeType]` 精确匹配 O(1)（`register(type:)` 预填 + 运行时缓存） ② `_userEntries` predicate scan（反向遍历，子类型优先）。原三层中的硬编码核心类型 is 链已删除，core type dispatcher 改由内部 plugin（CorePlugin 等）动态注册，统一入口。新增 `register(List<String> prefixes, {required Type type, bool Function(Object)? test})` 方法，`type`（必填）在注册时写入 `_exactMap` 实现 O(1) 精确匹配（与业界 VM 精确类型标识对齐：Dart VM CID / JVM klass / CPython ob_type）；`test`（可选）用于泛型/多态子类型兜底。未匹配到 `type` 的子类型（如 `_GrowableList<int>` vs `List`）通过 predicate scan fallback 命中 `is List`，结果缓存后后续也是 O(1)。HostClassRegistry 生命周期从 per-execute 创建改为由 DarticEngine 持有并传入解释器。
...
宿主回调内可再次调用 engine.call()。这是 host→dartic→host→dartic 链的核心场景。
...
| dartic 绑定文件数 | 29 个（core + async） | 当前实现 |
...
> **Phase 2**：多模块加载。loadBytecode 返回模块句柄，支持多个独立模块共存和按需替换。触发条件：需要模块化 dartic 架构。
...
| 组件 | 变更 | 说明 |
|------|------|------|
| HostClassRegistry | 简化为两层查找（exactMap + userEntries），删除硬编码 is-chain 和 10 个 late final 字段 | 新签名 `register(List<String> prefixes, {required Type type, bool Function(Object)? test})`；type 必填预热 _exactMap，test 可选用于泛型兜底 |
| HostClassRegistry | 生命周期从 per-execute 改为引擎持有 | DarticInterpreter 接受外部传入 |
...
| barrel file (dartic.dart) | 移除 HostBindingRegistry/HostClassRegistry/BridgeFactoryRegistry/DarticProxyManager 导出 | 新增 PluginContext 导出 |
...
| 公开 API | Ch4 内部组件 | 说明 |
|---------|-------------|------|
| DarticEngine | DarticInterpreter + HostBindingRegistry + HostClassRegistry + DarticProxyManager | 引擎封装所有内部组件 |
| DarticPlugin.register() | HostBindingRegistry.register() + HostClassRegistry 动态注册（Phase 7 新增） | 插件注册绑定和分发 |
| engine.call()（顶层） | executeFunction()（Phase 7 新增） | 按名查表 + 执行 |
| engine.call()（重入） | _runNestedDispatch()（Ch3 现有机制） | HOST_BOUNDARY 帧 + 嵌套 drive |
| registerClass(methods:) | HostBindingRegistry 批量注册 | 展平继承链的所有方法 |
| registerClass(test:, type:) | HostClassRegistry.register(exactType:)（Phase 7 新增） | 动态分发类型路由；exactType 实现注册顺序无关 |
...
| DarticProxy（自动） | DarticProxyManager.wrapForVM() | dartic 对象跨边界代理 |
| ClosureAdapter（自动） | ClosureAdapter.proxy0()..proxy6() | dartic 闭包跨边界回调代理 |
...
2. 声明 `@DarticExport(bridge: true) class ScreenBuilder`，允许 dartic 继承
...
**关键不变式**：dartic 的 HomeScreen 实例在 VM 侧是真正的 ScreenBuilder 子类，可直接参与 Flutter 框架的类型检查和 widget 构建。NavigationService 通过 @DarticExport 导出后，dartic 可正常实例化和调用其方法。
...
3. **Bridge 类**（@DarticExport(bridge: true)）→ `engine.registerClass(name, test, type, methods, bridgeFactory: factory)`
   - factory 签名匹配 Ch4 BridgeFactory：`(runtime, darticObj, superArgs) → Bridge`
   - 额外生成 super 转发器方法，注册到 HostBindingRegistry
