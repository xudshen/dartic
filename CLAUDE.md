...
- **排查必须编译器+解释器双视角** — 问题可能出在编译器（生成了错误字节码）或解释器（执行逻辑有误），排查时必须同时关注两端，不要只看一侧就下结论。用 dump CLI 对比预期字节码与实际字节码，再结合运行时日志定位根因。
- **系统性问题单独处理** — 修复过程中如果发现问题属于系统性缺陷（影响面广、需要架构层面调整），不要直接动手改，必须先和用户确认，走单独的调研（`docs/research/`）和修复方案（`docs/plans/`）流程。
- **问题追踪** — 发现待修复问题时（co19 triage、Flutter 实测、code review 等）：(1) 在 `docs/tasks/` 下创建 `dartic-<topic>.md` 问题文档（根因 + 方案）；(2) 如有 co19 skip 条目，注释指向该文档；(3) `docs/tasks/overview.md` "已知问题 TODO" 表格中添加条目（含来源、文档链接、优先级）。三处必须同步。
- **禁止手动修改 `.g.dart` 绑定文件** — 所有 `.g.dart` 是 `dartic gen` 自动生成的。如果绑定有问题，应修复 gen 工具（`packages/dartic_cli/` 下的代码生成逻辑）或调整 YAML 配置（`dartic_export.yaml`），然后重新运行 `dartic gen` 重新生成。
